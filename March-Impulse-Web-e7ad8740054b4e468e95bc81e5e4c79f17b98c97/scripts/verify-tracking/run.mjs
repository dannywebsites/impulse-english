...
//   node scripts/verify-tracking/run.mjs --popup      # + 45s CoursePopup submit test
//
...
async function main() {
  ...
  if (DO_POPUP) {
    ...
    // Must stay ahead of CoursePopup's DELAY_MS (45s) or this reports a false
    // "popup never appeared". Change both together.
    console.log('popup test: waiting 47s for the time-on-page popup…');
    ...
    results.push({
      page: POPUP_PAGE, label: 'CoursePopup submit (45s)', expect: 'generate_lead',
      sent: !!sent, tid: sent?.tid || '-', ghl: ghlOk ? '200✓' : 'FAIL',
