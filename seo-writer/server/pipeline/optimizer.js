...
/**
 * Determine which optimization steps are already completed.
 */
function getOptCompletedSteps(currentStep) {
  const stepOrder = [
    'scrape_complete',
    'keyword_complete',
    'research_complete',
  ...
  return new Set(stepOrder.slice(0, idx + 1));
}
...
export async function runOptimizationPipeline(jobId) {
  ...
  try {
    ...
    const completed = getOptCompletedSteps(job.currentStep);
    const isResuming = completed.size > 0;

    if (isResuming) {
      console.log(`[Optimizer] Resuming job ${jobId} from step "${job.currentStep}" — skipping ${completed.size} completed step(s)`);
    }

    await prisma.optimizationJob.update({
      where: { id: jobId },
      data: { action: 'Analyzing', errorMessage: null },
    });

    emitJobStatus(jobId, 'pipeline', 'running', isResuming ? 'Optimization pipeline resuming...' : 'Optimization pipeline started');

    // ── Step 1: Scrape ──
    let markdown, meta;

    if (completed.has('scrape_complete')) {
      console.log(`[Optimizer] Skipping scrape — already completed`);
      markdown = job.scrapedContent;
      meta = job.scrapedMeta;
    } else {
      const result = await runScrapeStep(job);
      markdown = result.markdown;
      meta = result.meta;

      console.log(`[Optimizer] Scrape saved. Cooling down ${STEP_DELAY_MS / 1000}s...`);
    ...
    if (completed.has('keyword_complete')) {
      console.log(`[Optimizer] Skipping keyword detection — already completed`);
      keyword = job.targetKeyword || job.detectedKeyword;
    } else {
      keyword = await runKeywordDetection(job, markdown);

      console.log(`[Optimizer] Keyword saved. Cooling down ${STEP_DELAY_MS / 1000}s...`);
      await sleep(STEP_DELAY_MS);
    }
    ...
    if (completed.has('research_complete')) {
      console.log(`[Optimizer] Skipping SERP research — already completed`);
      researchData = job.researchData;
    } else {
      researchData = await runSerpResearch(job, keyword);

      console.log(`[Optimizer] Research saved. Cooling down ${STEP_DELAY_MS / 1000}s...`);
      await sleep(STEP_DELAY_MS);
    }

    // ── Step 4: Analysis ──
    if (completed.has('analysis_complete')) {
      console.log(`[Optimizer] Skipping analysis — already completed`);
    } else {
    ...
    console.error(`[Optimizer] Error for job ${jobId} at step "${job?.currentStep || 'init'}":`, error);
