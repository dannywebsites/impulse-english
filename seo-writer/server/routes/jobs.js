...
// POST /api/jobs/:id/save-to-blog - Save completed article as markdown to the blog folder
router.post('/:id/save-to-blog', async (req, res) => {
  try {
    const { id } = req.params;
    const job = await prisma.contentJob.findUnique({ where: { id } });

    if (!job) return res.status(404).json({ error: 'Job not found' });
    if (!job.content) return res.status(400).json({ error: 'No content to save' });

    const slug = job.slug || job.topic.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
    ...
    const frontmatter = [
      '---',
      `title: "${(job.metatitle || job.topic).replace(/"/g, '\\"')}"`,
      `description: "${(job.metadescription || '').replace(/"/g, '\\"')}"`,
      `slug: "${slug}"`,
      `date: "${new Date().toISOString().split('T')[0]}"`,
      job.keywords ? `keywords: "${job.keywords}"` : null,
      '---',
    ].filter(Boolean).join('\n');
